...
DEFAULT_RUN_COMMAND = "apptainer exec --cleanenv --env USER=${OF2506_USER:-x-rkanak1} ${OF2506_IMAGE:-$HOME/openfoam-dev_2506.sif} bash -lc 'source /openfoam/bash.rc && export FOAM_SIGFPE=0 && export PYTHON=${PYTHON:-/home/x-rkanak1/.conda/envs/isw_env/bin/python3} && export MPLCONFIGDIR=${MPLCONFIGDIR:-/tmp} && ./Allrun'"
...
def peak_metrics(case_dir: Path, config: dict[str, object]) -> dict[str, float | str]:
    ...
    first_pulse = df[(df["time"] >= 0.0) & (df["time"] <= float(config["simulation_end_s"]) + 1e-12)].copy()
