...
def detect_surface_y_um(Yg: np.ndarray, Ag: np.ndarray, z_frac: float = 0.1) -> float:
    """Find Y of undisturbed metal surface from alpha=0.5 at far Z edges of YZ slice."""
    nz = Ag.shape[1]
    n_edge = max(1, int(z_frac * nz))
    surface_ys = []
    for col in range(n_edge):
        alpha_col = Ag[:, col]
        y_col = Yg[:, col]
        for i in range(len(alpha_col) - 1):
            a1, a2 = alpha_col[i], alpha_col[i + 1]
            if a1 == a2:
                continue
            if (a1 - 0.5) * (a2 - 0.5) <= 0:
                t = (0.5 - a1) / (a2 - a1)
                surface_ys.append(float(y_col[i] + t * (y_col[i + 1] - y_col[i])))
                break
    if not surface_ys:
        return 0.0
    return float(np.median(surface_ys))
...
def read_experimental_meltpool(exp_csv: Path) -> dict[str, float] | None:
    # Hardcoded from Riffel et al. (2026) weld12 earth condition
    return {
        "width_um": 114.6,
        "depth_um": 93.95,
    }
...
def plot_meltpool_comparison(
    ...
    err_lines = []
    for label, sim_val, exp_val in zip(categories, sim_values, exp_values):
        if np.isfinite(sim_val) and np.isfinite(exp_val) and exp_val != 0:
            err_pct = 100.0 * (sim_val - exp_val) / exp_val
            err_lines.append(f"{label}: {err_pct:+.1f}%")
    if err_lines:
        ax.text(
            0.02, 0.98,
            "\n".join(err_lines),
...
def measure(
    ...
    surface_y_um = detect_surface_y_um(Yg, Ag_yz)
    print(f"  Auto-detected surface_y_um = {surface_y_um:.2f} um")
