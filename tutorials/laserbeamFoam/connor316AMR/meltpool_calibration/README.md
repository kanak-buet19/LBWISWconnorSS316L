...
`Allrun` uses `/home/kanak/.venv310/bin/python` by default and forwards extra arguments.
...
```bash
bash -lc 'source /usr/lib/openfoam/openfoam2506/etc/bashrc; ./Allrun'
```

Override if needed:

```bash
/home/kanak/.venv310/bin/python calibrate_meltpool.py --run --run-command 'source /usr/lib/openfoam/openfoam2506/etc/bashrc; ./Allrun'
```

Prepare cases without running:

```bash
/home/kanak/.venv310/bin/python calibrate_meltpool.py
```

Run only the initial sensitivity grid, with no adaptive proposals:

```bash
/home/kanak/.venv310/bin/python calibrate_meltpool.py --run --no-adaptive
```

Use another config:

```bash
/home/kanak/.venv310/bin/python calibrate_meltpool.py --config my_config.json --run
```
