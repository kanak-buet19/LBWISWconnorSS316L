#!/bin/bash
#SBATCH --job-name=calib_validation
...
#   default is 8 cores/case, so 20 tasks runs 2 cases at once.
...
totalCores="${VALIDATION_TOTAL_CORES:-${SLURM_NTASKS:-20}}"
...
DEFAULT_CALIBRATED_CASES=(
    wolff_SS316_stationary_120W_r50um
...
if [ "${#setupArgs[@]}" -eq 0 ] && [ -n "${VALIDATION_CASES:-}" ]; then
    ...
    setupArgs=()
    for caseName in "${DEFAULT_CALIBRATED_CASES[@]}"; do
        setupArgs+=(--case "$caseName")
    done
fi
