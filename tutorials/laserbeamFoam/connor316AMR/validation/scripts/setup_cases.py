...
_PULSE_BEGIN = "// --- pulse settings (auto-generated by setup_cases.py) ---"
_PULSE_END = "// --- end pulse settings ---"


def configure_pulse(case_dir: Path, case: dict) -> None:
    """Write/refresh a pulseLaser block in LaserProperties from pulse_* keys.

    No-op (and strips any previously written block) if pulse_frequency_hz is
    absent, so continuous-wave cases are unaffected.
    """
    laser_props = case_dir / "constant" / "LaserProperties"
    text = read_text(laser_props)

    block_re = re.compile(
        rf"\n?{re.escape(_PULSE_BEGIN)}.*?{re.escape(_PULSE_END)}\n?",
        re.DOTALL,
    )
    text = block_re.sub("", text)

    freq = case.get("pulse_frequency_hz")
    if freq is None:
        write_text(laser_props, text)
        return

    duration = case.get("pulse_duration_s")
    if duration is None:
        raise RuntimeError(
            f"case {case['name']}: pulse_frequency_hz set but pulse_duration_s missing"
        )
    off_power = float(case.get("pulse_off_power_w", 0.0))

    lines = [
        _PULSE_BEGIN,
        "pulseLaser      true;",
        f"pulseFrequency  {float(freq):g};",
        f"pulseDuration   {float(duration):.6g};",
        f"pulseOffPower   {off_power:g};",
    ]
    if "pulse_start_s" in case:
        lines.append(f"pulseStart      {float(case['pulse_start_s']):.6g};")
    if "pulse_end_s" in case:
        lines.append(f"pulseEnd        {float(case['pulse_end_s']):.6g};")
    lines.append(_PULSE_END)
...
def configure_case(case_dir: Path, case: dict) -> None:
    ...
    write_text(
        ...
        f"    ({laser_off_time + 1e-6:.10e} 0)\n"
        f"    ({max(end_time, laser_off_time + 5e-6):.10e} 0)\n"
        ")\n",
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--dest-root",
        default=None,
        help="build cases directly under this dir instead of cases_dir "
             "(e.g. a per-run batch folder); relative paths resolve from the "
             "validation/ suite root",
    )
    args = parser.parse_args()

    cfg = load_config()
    template = ROOT / cfg.get("template", "template_case")
    cases_dir = (ROOT / args.dest_root) if args.dest_root else (ROOT / cfg.get("cases_dir", "cases"))
