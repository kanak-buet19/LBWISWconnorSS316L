...
# Single-case Anvil batch script for the connor316AMR template.
# Mirrors meltpool_calibration/calibrate_fine_job.sh:
...
#     laserbeamFoam (8 MPI ranks) and the host $PYTHON for meltpool analysis.
#
# Submit with:   sbatch job.sh
# Rank count is set by system/decomposeParDict (numberOfSubdomains = 8);
...
echo "=== Slurm connor316AMR bootstrap ==="
echo "Script: $0"
...
# Run from the case directory (where this script lives).
...
echo "requested Slurm tasks: ${SLURM_NTASKS:-unset}"
...
if [ "${DRY_RUN:-0}" = "1" ]; then
    ...
    echo "Would run: apptainer exec ... ./Allrun_long"
    echo "DRY RUN PASS: solver not started."
    exit 0
fi
