...
EXP_TIME_SHIFT_MS = 2.54
CASE_DIR = Path(__file__).resolve().parents[1]
KUBE_EXP_CSV_DIR = CASE_DIR.parent / "reference_data" / "csv"
ABSORPTIVITY_CSV = CASE_DIR / "absorptivity_vs_time" / "absorptivity_vs_time.csv"
LASER_DIAMETER_UM = 80.0
...
def plot_exp_comparison(
    ...
    current_time_ms_shifted = current_time_ms + exp_time_shift_ms
    ax.set_title(f"{title} at {current_time_ms_shifted:.3f} ms")
    ax.set_xlabel("Time (ms)")
    ax.set_ylabel(ylabel)
    ax.grid(True, alpha=0.3)
    ax.axvline(current_time_ms_shifted, color="black", linestyle=":", linewidth=1.2, alpha=0.8)

    sim_time_ms, sim_values = sim_series(rows_by_time, current_row, metric)
    finite = np.isfinite(sim_values)
    if np.any(finite):
        ax.plot(
            sim_time_ms[finite] + exp_time_shift_ms,
            sim_values[finite],
            color="#4C78A8",
            marker="o",
            markersize=3.0,
            linewidth=1.5,
            label="Simulation (shifted)",
        )
...
def plot_absorptivity_history(ax, absorptivity_csv: Path) -> None:
    ...
    if not absorptivity_csv.exists():
        ax.text(
            0.5,
            0.5,
            "Absorptivity CSV not found",
            transform=ax.transAxes,
            ha="center",
            va="center",
        )
    ...
    if missing:
        ax.text(
            0.5,
            0.5,
            "Absorptivity CSV missing columns",
            transform=ax.transAxes,
            ha="center",
            va="center",
        )
    ...
    handles, labels = ax.get_legend_handles_labels()
    if handles and data["laser"].nunique() > 1:
        ax.legend(handles, labels, frameon=True, facecolor="white", edgecolor="black", fontsize=8)
...
def list_vtk_files(vtk_dir: Path) -> list[Path]:
    vtu_files = list(vtk_dir.glob("*/internal.vtu"))
    legacy_files = list(vtk_dir.glob("*.vtk"))
    return sorted(vtu_files + legacy_files, key=vtk_time)


def section_index(vtk_file: Path, vtk_dir: Path) -> int:
    vtk_files = list_vtk_files(vtk_dir)
...
def main() -> None:
    ...
    parser.add_argument("--exp-csv-dir", type=Path, default=KUBE_EXP_CSV_DIR)
