...
)
{
    ...
    if (Pstream::master())
    {
        ...
        fileName absorptivityDir;

        if (Pstream::parRun())
        {
            absorptivityDir = runTime.path()/".."/"absorptivity_vs_time";
        }
        else
        {
            absorptivityDir = runTime.path()/"absorptivity_vs_time";
        ...
        mkDir(absorptivityDir);

        const fileName csvFile =
            absorptivityDir/"absorptivity_vs_time.csv";
        const bool newFile = !isFile(csvFile);
        ...
        if (newFile)
        {
            os  << "time_s,laser,input_power_W,deposited_power_W,"
                << "absorptivity" << nl;
        }

        const scalar absorptivity =
            currentLaserPower > SMALL ? TotalQ/currentLaserPower : 0.0;
